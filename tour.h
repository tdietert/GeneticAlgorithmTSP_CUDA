...
struct tour_t{
    ...
    __host__ __device__ tour_t(const tour_t& copyFrom)
    {
        for (int i = 0; i < NUM_CITIES; ++i)
        {
          cities[i] = copyFrom.cities[i];
        }
...
__host__ __device__ void evalTour(tour_t &tour)
{
    tour.distance = 0;
    for (int i = 0; i < NUM_CITIES; ++i)
    {
        if (i < NUM_CITIES - 1)
            tour.distance += distBetweenCities(tour.cities[i], tour.cities[i+1]);
        else 
            tour.distance += distBetweenCities(tour.cities[i], tour.cities[0]);
    }
